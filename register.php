...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $username = htmlspecialchars($_POST["username"]);   // beholdt som før
    $password = $_POST["password"] ?? "";              // IKKE htmlspecialchars på password

    if (strlen($username) < 3 || strlen($username) > 14) {
        $error = "Username must be between 3 and 14 characters";
    } else {
        // NYT (minimalt): gem i din eksisterende tabel `login` med hash
        $hash = password_hash($password, PASSWORD_DEFAULT);
        $db->sql(
                "INSERT INTO login (loginNavn, loginKodeord) VALUES (:u, :p)",
                [":u" => $username, ":p" => $hash]
        );

        // DIT EKSISTERENDE FLOW:
